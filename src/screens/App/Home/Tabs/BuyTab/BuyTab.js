...
import BuyMainTab from '../BuyMainTab/BuyMainTab';
...
const DreamAddress = () => {
  const { dream_addresses } = useSelector(state => state?.appReducer)
  ...
  const AddressesRow = ({ item, index }) => {
    ...
    return (
      <View style={styles.addressItemRow(index)}>
        <Image source={appIcons.location} style={styles.locationIconStyle} />
...
const Map = () => {
  ...
  return (
    <>
      <Text style={{ color: colors.b1, fontFamily: family.Gilroy_Medium, fontSize: size.xlarge }} >Draw On Map</Text>
      ...
      <Text style={{ color: colors.b1, fontFamily: family.Gilroy_Medium, fontSize: size.xlarge }} >Saved Locations</Text>
      <SavedLocations />
    </>
  )
}

const BuyTab = ({ selected, setSelected }) => {
  ...
  return (
    <>
      <View style={styles.paddingView}>
        {['preference', 'map'].includes(selected) &&
          <View style={{ flexDirection: 'row', alignItems: 'center', marginVertical: WP(4) }} >
            <TouchableOpacity style={{ marginRight: WP(2) }} onPress={() => setSelected('buy')}>
              <Icon
                type={'antdesign'}
                name={'arrowleft'}
                size={22}
            ...
            <Text style={styles.propertyTxtStyle} numberOfLines={1}>
              {selected == 'preference' ?
        ...
        {selected == 'buy' && <BuyMainTab />}
        {selected == 'preference' && <Preference />}
        {selected == 'dreamAddress' && <DreamAddress />}
        {selected == 'map' && <Map />}
      </View >
